def training_data(df_cov, outcomestring):
    ...
    patients_vec = []
    patients_label = []
    Seq_len =[] #store the #visits for each patient
    # print(n_patient)
    for i in range(n_patient):
        ...
        for j in range(len(p)):
            ...
            temp = p.iloc[j]
            temp = temp.fillna(0)
            # temp = 0 if str(temp) == 'N/A' else temp
...
def dataaugmentation(patients_vec, patients_label):
    ...
    for i in range(len(patients_vec)): ## patient-wise
        for j in range(len(patients_vec[i])):
...
def testing_data(df_cov, outcomestring):
    ...
    for i in range(n_patient):
        ...
        for j in range(len(p)):
            ...
            temp = temp.drop("Elapsed time since first imaging")
            temp = temp.drop("Fold number")
            temp = temp.drop("Number previous visits")
            temp = temp.drop("Contralateral eye moths wet")
            temp = temp.drop("Number averaged scans")
            temp = temp.drop("Contralateral eye status")
