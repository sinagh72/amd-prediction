...
class SelfAttention(nn.Module):
    ...
    def forward(self, values, keys, query, mask):
        #number of examples we are sending
        N = query.shape[0]
        #correspond to src len, trg len
        ...
        # queries shape: (N, query_en, heads, heads_shape)
        # keys shape: (N, key_len, heads, heads_dim)
        # enerygy shape: (N, heads, query_len, key_len)

        if mask is not None:
            energy = energy.masked_filled(mask == 0, float("-1e20"))
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, value, key, query, mask):
        ...
        out = self.dropout(self.nom2(forward + x))
        return  out
...
class Decoder(nn.Module):
    def __int__(self, trg_vocab_size,
                embed_size,
                num_layers,
                heads,
                forward_expansion,
        ...
        self.device = device
        self.forward_embedding = nn.Embedding(trg_vocab_size, embed_size)
