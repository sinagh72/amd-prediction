...
def model_training(df_train, m, fold, n, flag, strm, dir_name, percentage, padding="pre"):
    ...
    x_train = pad_sequences(x_train_aug, slen, padding=padding, truncating=padding, value=0, dtype='float32')
    y_train = pad_sequences(y_train_aug, slen, padding=padding, truncating=padding, value=2.)

    x_val = pad_sequences(patients_vec_val, slen, padding=padding, truncating=padding, value=0, dtype='float32')
    y_val = pad_sequences(patients_label_val, slen, padding=padding, truncating=padding, value=2.)
...
def model_using(df_train, fold, n, strm, model_path, padding):
    train = df_train[df_train['Fold number'] % 5 != fold - 1]
    train = train.reset_index(drop=True)
    patients_vec_train, patients_label_train, Seq_len = training_data(train, strm)
    print("#train: ", len(patients_vec_train))
